...
class project_cat_request extends FormRequest
{
    /**
    ...
    {
        if(Gate::any(["create_project_cat","update_project_cat"])){
            return true;
        }else{
            return false;
        }
    ...
    public function rules(): array
    {
        $rules=[
            ...
            'seo_url' => ['required', 'string', 'min:1', 'max:255','unique:project_cats,seo_url'],
        ...
        if(isset($this->id)){
            $rules['seo_url']=['required', 'string', 'min:1', 'max:255','unique:project_cats,seo_url,'.$this->id];
