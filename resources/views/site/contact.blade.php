...
<div class="content-wrap ">
    <div class="position-relative py-210">
        <div class="background">
            <div class="background-image jarallax" data-jarallax data-speed="0.8">
                <img class="jarallax-img" loading="lazy" @if($module_pic) src="{{asset("upload/".$module_pic)}}"
                @else src="{{asset("site/assets/img/contact-hero-1920x900.jpg")}}" @endif alt="{{$module_title}}">
            </div>
    ...
    <div class="pt-120 pb-180 footerPrev show_here">
        <div class="container">
            <div class="row gy-50">
                <div class="col-lg-4">
                    @if($email && $tell)
                    <div class="d-flex" data-show="startbox">
                        <div class="flex-shrink-0">
                            <svg class="text-accent-1" xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="none">
                        ...
                        <div class="flex-grow-1 me-15">
                            <h6 class="m-0 ff-custom fw-normal">اطلاعات تماس:</h6>
                            <p class="m-0 mt-15">
                                @if($email) ایمیل: {{$email}} @endif
                                @if($tell)<br>تلفن:  <a href="tel:{{$tell}}" style="color: #000">{{$tell}}</a>@endif
                            </p>
                        </div>
                    </div>
                    @endif
                    ...
                    <div class="d-flex mt-50" data-show="startbox" data-show-delay="100">
                        <div class="flex-shrink-0"><svg class="text-accent-1" xmlns="http://www.w3.org/2000/svg" width="15" height="17" fill="none">
                                <path fill="currentColor" fill-rule="evenodd" d="M7.5 17S15 12.364 15 6.955c0-1.845-.79-3.614-2.197-4.918C11.397.733 9.49 0 7.5 0 5.51 0 3.603.733 2.197 2.037.79 3.34 0 5.11 0 6.955 0 12.364 7.5 17 7.5 17ZM10 7.286c0 1.341-1.12 2.428-2.5 2.428S5 8.627 5 7.286c0-1.342 1.12-2.429 2.5-2.429S10 5.944 10 7.286Z" clip-rule="evenodd" />
                            </svg></div>
                        <div class="flex-grow-1 me-15">
                            <h6 class="m-0 ff-custom fw-normal">آدرس:</h6>
                            <p class="m-0 mt-15">{{$address}}</p>
                        </div>
                    </div>
                    @endif
                    @if($working_time)
                    <div class="d-flex mt-50" data-show="startbox" data-show-delay="200">
                        <div class="flex-shrink-0"><svg class="text-accent-1" xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="none">
                                <path fill="currentColor" fill-rule="evenodd" d="M16 8A8 8 0 1 1 0 8a8 8 0 0 1 16 0ZM8.7 3.379a1 1 0 1 0-2 0v4.8a1 1 0 0 0 .553.894l3.2 1.6a1 1 0 1 0 .894-1.789L8.7 7.561V3.379Z" clip-rule="evenodd" />
                            </svg></div>
                        <div class="flex-grow-1 me-15">
                            <h6 class="m-0 ff-custom fw-normal">ساعات فعالیت:</h6>
                            <p class="m-0 mt-15">{{$working_time}}</p>
                        </div>
                    </div>
                    @endif
                </div>
                ...
                @endif
                    <form action="{{route("contact.store")}}" method="post" class="form">
                        ...
                        <div class="row gy-40">
                            ...
                            @component('site.components.input',['name'=>'mobile','placeholder'=>'شماره تماس *','type'=>'tel','value'=>old('mobile')])@endcomponent
                            @component("site.components.select_recursive",['name'=>'catid','placeholder'=>'واحد مربوطه','options'=>$message_cats, 'sub_method'=>'sub_cats','value'=>old('catid'),'first_option'=>'واحد مربوطه'])@endcomponent
                            @component("site.components.textarea",['name'=>'note','placeholder'=>'متن درخواست *','value'=>old('note')])@endcomponent
                            <div class="col-12 col-lg-3 text-end text-lg-start">
                                <button type="submit" class="btn btn-accent-1">ارسال</button>
                            </div>
                        </div>
                    </form>
                </div>
    ...
    <div class="position-relative">
        <div class="half-section-block container position-absolute top-50 start-0 end-0" data-prev-id=".footerPrev" data-next-id=".footerNext">
            <div class="shape-parent">
                <div class="shape justify-content-end mt-n40 ms-n45 text-dark" data-jarallax-element="40">
                ...
                <div class="map-container bg-dark rounded-4 overflow-hidden" style="height: 506px;" id="map">
            </div>
        </div>
    </div>
</div>
