                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            @component($prefix_component."navtab",['number'=>2,'titles'=>['لیست','جستجو']])
                                @slot("tabContent0")
                                    @if(isset($project[0]))
                                        @component($prefix_component."form",['action'=>route('admin.project.action_all')])
                                            @slot("content")
                                                <table class="table text-center">
                                                    ...
                                                    <tr>
                                                        ...
                                                        @canany(["delete_project","update_project","read_content"])
                                                    ...
                                                    @foreach($project as $item)
                                                        <tr>
                                                            ...
                                                            <td>{{ $loop->iteration + $project->firstItem() - 1 }}
                                                            <td>{{$item["title"]}}</td>
                                                            <td>مدیر اصلی</td>
                                                            <td>{{$item->project_cat->title ?? ""}}</td>
                                                            ...
                                                            <td>
                                                                @can("update_project")
                                                                <a href="{{route("admin.project.edit",['project'=>$item['id']])}}" class="btn btn-success btn-sm"><i class="fas fa-edit"></i></a>
                                                                @endcan
                                                                @can("delete_project")
                                                                <a href="javascript:void(0)" data-href="{{route("admin.project.destroy",['project'=>$item['id']])}}" class="btn btn-danger btn-sm delete">
                                                                    <i class="fas fa-trash"></i>
                                                                </a>
                                                                @endcan
                                                                @can("read_content")
                                                                <a href="{{route("admin.content.list",['item_id'=>$item['id'],'module'=>'project'])}}" class="btn btn-primary btn-sm"> محتوا
                                ...
                                @slot("tabContent1")
                                    @component($prefix_component."form",['method'=>'get'])
                                        @slot("content")
                                            ...
                                            @component($prefix_component."select_recursive",['name'=>'catid','value'=>request()->get('catid'),'options'=>$project_cats_search,'label'=>'دسته بندی','choose'=>true, 'sub_method'=>'sub_cats'])@endcomponent
                                            <div class="my-3">
